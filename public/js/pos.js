$(document).ready(function() {
    ...
    $('#customer_id').select2({
        ...
        templateResult: function (data) { 
            ...
            template += data.text + "<br>" + LANG.mobile + ": " + data.mobile + "<br>" + LANG.address1 + ": " + data.address_line_1 +"<br>"+ LANG.address2 + ": " + data.address_line_2;
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        if ($('.contact_address1').length) {
            get_contact_due(data.id);
        }
        if ($('.contact_address2').length) {
            get_contact_due(data.id);
        }
    });
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result != '') {
                    ...
                    $('.contact_name').find('span').text(element.name);
                    $('.contact_name').removeClass('hide');
                    $('.contact_phone').find('span').text(element.mobile);
                    $('.contact_phone').removeClass('hide');
                    $('.contact_address1').find('span').text(element.address_line_1);
                    $('.contact_address1').removeClass('hide');
                    $('.contact_address2').find('span').text(element.address_line_2);
                    $('.contact_address2').removeClass('hide');
                    });
                    
                } else {
                    $('.contact_name').find('span').text('');
                    $('.contact_name').removeClass('hide');
                    $('.contact_phone').find('span').text('');
                    $('.contact_phone').removeClass('hide');
                    $('.contact_address1').find('span').text('');
                    $('.contact_address1').removeClass('hide');
                    $('.contact_address2').find('span').text('');
                    $('.contact_address2').removeClass('hide');
                }
            },
        });
...
function reset_pos_form(){
    ...
    $('.contact_name').find('span').text('');
    $('.contact_name').addClass('hide');
    $('.contact_phone').find('span').text('');
    $('.contact_phone').addClass('hide');
    $('.contact_address1').find('span').text('');
    $('.contact_address1').addClass('hide');
    $('.contact_address2').find('span').text('');
    $('.contact_address2').addClass('hide');
...
$(document).on('click', '.edit_this_customer', function() {
    $.ajax({
        type: "get",
        url: "/contacts/"+$("#customer_id").val()+"/edit",
        success: function (response) {
            $('#edit_contact_container').html(
                '<div class="modal fade contact_modal_edit" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">'
                + response +
                "</div>"
            );
            $('#customer_id').select2('close');
            ...
            $('.contact_modal_edit')
                ...
                .closest('div.contact_type_div')
                .addClass('hide');
            $('.contact_modal_edit').modal('show');
        }
    });

});
$('.product_category_item ,.product_sub_category_item').on('click', function(e) {
    $('input#suggestion_page').val(1);
    var location_id = $('input#location_id').val();
    if (location_id != '' || location_id != undefined) {
        get_product_suggestion_list(
            $(this).attr("category-id"),
            $('select#product_brand').val(),
            $('input#location_id').val(),
            null
        );
