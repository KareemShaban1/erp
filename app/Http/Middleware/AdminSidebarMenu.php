            ...
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view') || auth()->user()->can('supplier.view_own') || auth()->user()->can('customer.view_own')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('supplier.view') || auth()->user()->can('supplier.view_own')) {
                            $sub->url(
                                action('ContactController@index', ['type' => 'client']),
                                __('report.client'),
                                ['icon' => 'fa fas fa-star', 'active' => request()->input('type') == 'client']
                            );
                        }
                        if (auth()->user()->can('supplier.view') || auth()->user()->can('supplier.view_own')) {
                            $sub->url(
                                action('ContactController@index', ['type' => 'delivery']),
                                __('report.delivery'),
                                ['icon' => 'fa fas fa-star', 'active' => request()->input('type') == 'delivery']
                            );
                        }
                        ...
                        dd(dd(config('app.GOOGLE_MAP_API_KEY')));
